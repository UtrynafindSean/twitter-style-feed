...
router.post("/:postId/like", async (req, res) => {
  try {
    ...
    const alreadyLiked = likes.some((id) => String(id) === String(userId));
...
router.post("/:postId/repost", async (req, res) => {
  try {
    ...
    const alreadyReposted = reposts.some((id) => String(id) === String(userId));

    if (alreadyReposted) {
      await Post.updateOne({ _id: postId }, { $pull: { reposts: userId } });
    } else {
      await Post.updateOne({ _id: postId }, { $addToSet: { reposts: userId } });
    }
